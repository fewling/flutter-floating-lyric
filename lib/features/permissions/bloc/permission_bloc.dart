...
class PermissionBloc extends Bloc<PermissionEvent, PermissionState> {
  PermissionBloc({
    ...
    on<PermissionEvent>(
      (event, emit) => switch (event) {
        ...
        NotificationListenerRequested() => _onNotificationListenerRequested(
          event,
          emit,
        ),
        SystemAlertWindowRequested() => _onSystemAlertWindowRequested(
          event,
          emit,
        ),
      },
  ...
  Future<void> _onInitial(
    PermissionEventInitial event,
    Emitter<PermissionState> emit,
  ) async {
    final isListenerGranted = await _permissionService
        .checkNotificationListenerPermission();
    final isWindowGranted = await _permissionService
        .checkSystemAlertWindowPermission();

    logger.d(
      'isListenerGranted: $isListenerGranted, isWindowGranted: $isWindowGranted',
    );
  ...
  void _onNotificationListenerRequested(
    NotificationListenerRequested event,
    Emitter<PermissionState> emit,
  ) {
    _platformMethodService.requestNotificationListenerPermission();
  }

  void _onSystemAlertWindowRequested(
    SystemAlertWindowRequested event,
    Emitter<PermissionState> emit,
  ) {
